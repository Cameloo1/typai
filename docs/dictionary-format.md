...
## Production Transform Pipeline

Prompt 131 provides the deterministic production transform pipeline:
...
The production build command reads
`packages/core/assets/production/MANIFEST.json` when present, otherwise
`packages/core/assets/production/MANIFEST.template.json`. It fails before
processing when `review.status` is `blocked`, when source hashes are missing or
mismatched, when pinned local source files are absent, or when required
license/attribution files are absent in an approved manifest.

While production ingestion is blocked, the validation command runs the same
normalization, frequency merge, protected-token filtering, de-duplication,
sorting, blob encoding, and metadata generation path against repo-local fixture
inputs under:

- `packages/core/assets/fixtures/production-transform/`

Generated fixture outputs are written under `packages/core/assets/generated/`,
which is ignored by Git. The transform does not fetch from the network by
default. Approved production manifests must provide pinned local input file
paths and SHA-256 hashes before processing can begin.

The dictionary parser accepts text/Hunspell `.dic` rows and selected `.dic`
entries inside ZIP archives. The frequency parser accepts fixture TSV rows and
streams Google Books Ngram gzip partitions line by line. `totalcounts-1` is
verified as source metadata but is not parsed as a word-frequency table.

Generated metadata records:

- schema version and Blob v1 asset schema version
- input manifest SHA-256
- source file hashes and byte sizes
- transform script version and SHA-256
- word count and generated byte size
...
Normalization is ASCII-only for Blob v1: trim, screen protected-looking tokens,
strip simple Hunspell flags, normalize with NFKC, lowercase with `en-US`, allow
only `a-z`, reject apostrophes, hyphens, trailing punctuation, non-ASCII,
identifiers, URLs, emails, paths, and numeric tokens, then sort accepted words
by ASCII lexicographic order.
...
## Runtime Loading Hardening

Prompt 133 confirms the Blob v1 path through TypeScript, Rust/Wasm, and C++:

- `dictionary.bytes`, `dictionary.load`, and `dictionary.url` are resolved only
  during `createTypaiCore()` initialization.
- `checkCompletedToken()` and `suggestToken()` remain synchronous after
  initialization and do not fetch, parse, or reload assets.
- `production` mode fails with a clear unavailable error while the production
  manifest remains blocked.
- `host-provided`, `fixture`, and `scaled-mock` modes all require an explicit
  initialization source and use the same C++ Blob v1 validator.
- failed production or malformed host-provided loads do not replace the
  currently loaded C++ dictionary or delete-index state.
- `built-in` mode and default initialization clear dynamic dictionary state and
  use the tiny deterministic built-in vocabulary.

The C++ FFI surface remains pointer-and-length based. It exposes primitive
counts and byte-size stats only; no `std::string`, C++ object ownership,
classes, Embind/Emscripten binding helpers, or full-document text APIs cross
the boundary. `pnpm ffi:audit` checks those constraints.
