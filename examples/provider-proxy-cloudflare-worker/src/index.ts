...
import {
  type ProviderProxyWorkerEnv,
  type ValidatedProviderProxyWorkerEnv,
  validateEnv,
} from "./validateEnv";

export type TypaiWorkerHandlerOptions = {
  simulateProviderFailure?: boolean;
};

export async function handleTypaiWorkerRequest(
  request: Request,
  env: ProviderProxyWorkerEnv = {},
  options: TypaiWorkerHandlerOptions = {},
): Promise<Response> {
  let config: ValidatedProviderProxyWorkerEnv;
...
const worker = {
  async fetch(request: Request, env: ProviderProxyWorkerEnv): Promise<Response> {
    return handleTypaiWorkerRequest(request, env);
  },
};
