...
const runtimeScenarios = [
  ...
  "load host-provided dictionary bytes",
  "verify blocked production dictionary mode",
...
function createRuntimeSmokeScript() {
  return [
    ...
    "});",
    'assert(hostProvidedTypai.getLoadedDictionaryWordCount() === 3, "Expected host-provided dictionary words to load.");',
    "assert(",
    "  hostProvidedTypai.getLoadedDictionaryByteSize() === hostProvidedBytes.byteLength,",
    '  "Expected host-provided dictionary byte-size stat.",',
    ");",
    "assert(",
    '  hostProvidedTypai.checkCompletedToken({ token: "receipt" }).action === "do_nothing",',
    '  "Expected host-provided word to be known.",',
    ");",
    "assert(",
    '  hostProvidedTypai.suggestToken({ token: "reciept", maxSuggestions: 3 }).suggestions.includes("receipt"),',
    '  "Expected host-provided dictionary suggestion.",',
    ");",
    "",
    ...
    "  })",
    "  .catch((error) => {",
    '    assert(/production dictionary asset is unavailable/i.test(String(error?.message ?? error)), "Expected clear production-mode unavailable error.");',
    "  });",
    "",
    ...
    "    for (const file of files) {",
    '      assert(!file.startsWith("assets/"), "@typai/core contains blocked asset path " + file + ".");',
    '      assert(!/production.*(?:dictionary|frequency)/i.test(file), "@typai/core contains blocked production asset " + file + ".");',
    '      assert(!/\\.(?:aff|dic|gz|tsv|zip)$/i.test(file), "@typai/core contains raw dictionary/frequency source " + file + ".");',
    '      assert(!/(?:^|\\/)totalcounts-\\d+$/i.test(file), "@typai/core contains raw frequency source " + file + ".");',
    "    }",
    "  }",
    "}",
    "",
